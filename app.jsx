...
const {
      Globe,  Sun,  Moon,  Menu,  X,  CheckCircle,     Instagram,  Youtube,  Image as ImageIcon,  ChevronRight,     Settings,  ShoppingBag,  LayoutDashboard,  AlertCircle,  Phone,  Lock 
} = Icons;
  ...
  useEffect(() => {
    const initAuth = async () => {
      if (typeof __initial_auth_token !== 'undefined' && __initial_auth_token) {
        ...
        setIsAdmin(true); // Treat custom token users
      } else {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-gray-500">You must be logged in/p>
        </div>
      </div>
    );
  }
